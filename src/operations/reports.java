...
public class reports {
	...
	public static ArrayList<HashMap<String, Integer>> rentersRanking(Connection connection, Date start, Date end) throws SQLException{
		...
		query = "SELECT name FROM user, history FROM user, history WHERE user.SIN=history.renterSIN AND startDate>='"+start+"' AND endDate<='"+end+"'GROUP BY name";
		ResultSet data = DBAPI.getDataByQuery(connection, query);
	...
	public static HashMap<String, Integer> largestHost(Connection connection, Date start) throws SQLException{
		...
		query = "SELECT name, COUNT(*) AS COUNT FROM user U,cancelled where U.SIN=cancelled.hostSIN AND (SELECT COUNT(*) FROM user,cancelled WHERE startDate > '"+start+"' AND U.SIN=cancelled.hostSIN AND who='host')>all(SELECT COUNT(*) FROM user,cancelled WHERE startDate > '\"+start+\"' AND user.SIN=cancelled.hostSIN AND who='host' GROUP BY user.name)";
		ResultSet data = DBAPI.getDataByQuery(connection, query);
	...
	public static HashMap<String, Integer> largestRenter(Connection connection, Date start) throws SQLException{
		HashMap<String, Integer> result = new HashMap<String, Integer>();
		query = "SELECT name, COUNT(*) AS COUNT FROM user U,cancelled where U.SIN=cancelled.renterSIN AND (SELECT COUNT(*) FROM user,cancelled WHERE startDate > '"+start+"' AND U.SIN=cancelled.renterSIN AND who='renter')>all(SELECT COUNT(*) FROM user,cancelled WHERE startDate > '\"+start+\"' AND user.SIN=cancelled.renterSIN AND who='renter' GROUP BY user.name)";
		ResultSet data = DBAPI.getDataByQuery(connection, query);
	...
	public static ArrayList<HashMap<String, Integer>> wordCloud(Connection connection) throws SQLException{
		...
		query = "SELECT message, listingID FROM listingRating GROUP BY listingID";
		ResultSet data = DBAPI.getDataByQuery(connection, query);
