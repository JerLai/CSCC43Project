...
public class reports {
	...
	public static HashMap<String, Integer> largestHost(Connection connection, Date start) throws SQLException{
		...
		query = "SELECT name, COUNT(*) AS COUNT FROM users U,cancelled WHERE hostCancel='1' AND U.SIN=cancelled.hostSIN AND (SELECT COUNT(*) FROM users,cancelled WHERE startDate > '"+start+"' AND U.SIN=cancelled.hostSIN)>all(SELECT COUNT(*) FROM users,cancelled WHERE startDate > '"+start+"' AND users.SIN=cancelled.hostSIN GROUP BY user.name)";
		ResultSet data = DBAPI.getDataByQuery(connection, query);
	...
	public static HashMap<String, Integer> largestRenter(Connection connection, Date start) throws SQLException{
		HashMap<String, Integer> result = new HashMap<String, Integer>();
		query = "SELECT name, COUNT(*) AS COUNT FROM users U,cancelled WHERE hostCancel='0' AND U.SIN=cancelled.renterSIN AND (SELECT COUNT(*) FROM users,cancelled WHERE startDate > '"+start+"' AND U.SIN=cancelled.renterSIN)>all(SELECT COUNT(*) FROM user,cancelled WHERE startDate > '\"+start+\"' AND user.SIN=cancelled.renterSIN GROUP BY user.name)";
		ResultSet data = DBAPI.getDataByQuery(connection, query);
